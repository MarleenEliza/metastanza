<template>
  <div class="tableOption">
    <form class="textSearchWrapper" @submit.prevent="state.queryForAllColumns.commit()">
      <input
    ...
    <thead>
      <tr>
        <th v-for="(column, i) in state.columns" :key="column.id">
          ...
          ></span>
          <span
            :class="['icon', 'searchIcon', { active: column.isSearchConditionGiven }]"
  ...
  setup(params) {
    ...
    const filteredRows = computed(() => {
      ...
      const filtered = state.allRows.filter((row) => {
        return searchByAllColumns(row, queryForAllColumns) && searchByEachColumn(row);
      });
    ...
    const isModalShowing = computed(() => {
      return state.columns.some(({isSearchModalShowing}) => isSearchModalShowing);
    });

    const isPopupOrModalShowing = computed(() => {
      return state.columns.some(({isFilterPopupShowing}) => isFilterPopupShowing) || isModalShowing.value;
    });
    ...
    function closeModal() {
      for (const column of state.columns) {
        column.isFilterPopupShowing = false;
        column.isSearchModalShowing = false;
      }
    }
...
function createColumnState(columnDef, values) {
  ...
  if (columnDef.type === "number") {
    ...
    const filters = uniq(values).sort().map((value) => {
      return reactive({
        value,
        checked: true,
      });
